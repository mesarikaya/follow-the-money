...
public class MacroIngestionHandler {
  ...
  private int fetchSeries(String seriesId, LocalDate today, List<String> errors) {
    try {
      LocalDate from =
          macroRepo
              .findMaxObservationDate(seriesId)
              .map(d -> d.plusDays(1))
              .orElse(today.minusYears(BACKFILL_YEARS));
      if (!from.isBefore(today)) return 0;

      List<MacroIndicatorRepository.Row> rows =
          fredClient.fetchObservations(seriesId, from, today).stream()
  ...
  private MacroIndicatorRepository.Row toRow(
      String seriesId, FredObservationsResponse.Observation o) {
    return new MacroIndicatorRepository.Row(
        LocalDate.parse(o.date()), seriesId, new BigDecimal(o.value()));
  }
}
