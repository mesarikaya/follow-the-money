...
class IngestLogRepositoryIT {
  ...
  @Autowired IngestLogRepository repository;
  ...
  void shouldPersistErrorsJson() {
    ...
    repository.update(
        log.finish(OffsetDateTime.now(), IngestStatus.PARTIAL, 5, "[\"T10Y2Y: timeout\"]"));
  ...
  void shouldReturnOneLogPerSource() {
    ...
    assertThat(latest)
        .extracting(IngestLog::source)
        .containsExactlyInAnyOrder(IngestSource.PRICES, IngestSource.MACRO);
    assertThat(
            latest.stream()
                .filter(l -> l.source() == IngestSource.PRICES)
                .findFirst()
                .orElseThrow()
                .runId())
