...
class ThemePeerDivergenceAlertEvaluatorTest {

  private static final LocalDate DATE = LocalDate.of(2024, 6, 1);
  private static final String RULE = "theme_peer_divergence";
  ...
  private ThemePeerDivergenceAlertEvaluator evaluator() {
    return new ThemePeerDivergenceAlertEvaluator(
  ...
  @Test
  void firesWhenSpreadWideAndThemeActive() {
    ...
    // spread = 0.80 − 0.40 = 0.40 (> 0.30), avg ≈ 0.55 (> 0.40).
    when(signalRepository.findByTypeAndDate(SignalType.COMPOSITE, DATE))
        .thenReturn(
            Map.of("A", new BigDecimal("0.80"), "B", new BigDecimal("0.40"), "C", new BigDecimal("0.45")));
  ...
  @Test
  void doesNotFireBelowMinimumConstituents() {
    when(alertRulesRepository.findById(RULE)).thenReturn(Optional.of(rule(true)));
    when(themeRepository.findAllConstituentsByTheme()).thenReturn(Map.of("AI", List.of("A", "B")));
    when(signalRepository.findByTypeAndDate(SignalType.COMPOSITE, DATE))
        .thenReturn(Map.of("A", new BigDecimal("0.80"), "B", new BigDecimal("0.40")));
    lenient().when(alertRepository.existsActiveAlertForTheme(RULE, "AI")).thenReturn(false);
