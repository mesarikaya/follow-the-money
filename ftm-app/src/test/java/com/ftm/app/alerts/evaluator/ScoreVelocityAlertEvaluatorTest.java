...
class ScoreVelocityAlertEvaluatorTest {

  private static final LocalDate DATE = LocalDate.of(2024, 6, 1);
  private static final String RULE = "score_velocity";

  @Mock AlertRulesRepository alertRulesRepository;
  @Mock SignalRepository signalRepository;
  @Mock AlertRepository alertRepository;

  private ScoreVelocityAlertEvaluator evaluator() {
    return new ScoreVelocityAlertEvaluator(alertRulesRepository, signalRepository, alertRepository);
  }
  ...
  @Test
  void firesSurgeWhen5dTrendAboveThreshold() {
    ...
    when(signalRepository.findByTypeAndDate(SignalType.COMPOSITE_TREND_5D, DATE))
        .thenReturn(Map.of("TECH", new BigDecimal("0.15")));
  ...
  @Test
  void doesNotFireWhenTrendWithinNormalBand() {
    when(alertRulesRepository.findById(RULE)).thenReturn(Optional.of(rule(true)));
    when(signalRepository.findByTypeAndDate(SignalType.COMPOSITE_TREND_5D, DATE))
