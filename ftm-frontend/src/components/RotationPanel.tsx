          ...
          {rotation.topLeaders.map((leader) => (
            <li key={leader.categoryId} className="flex flex-col gap-1">
              <div className="flex items-center justify-between">
                <span className="text-xs font-medium text-zinc-200">{leader.categoryName}</span>
                <span className="text-xs text-zinc-500">{leader.categoryId}</span>
              </div>
        ...
        <ul className="space-y-2">
          ...
          {rotation.bottomLaggards.map((laggard) => (
            <li key={laggard.categoryId} className="flex flex-col gap-1">
              <div className="flex items-center justify-between">
                <span className="text-xs font-medium text-zinc-200">{laggard.categoryName}</span>
                <span className="text-xs text-zinc-500">{laggard.categoryId}</span>
              </div>
              <div className="flex items-center gap-2">
      ...
      <div className="bg-zinc-800/50 border border-zinc-700/50 rounded-lg p-4">
        ...
        <ul className="space-y-2">
          {rotation.recentEvents.length === 0 && (
            <li className="text-xs text-zinc-500">No rotation events in the last 90 days</li>
          )}
          {rotation.recentEvents.slice(0, 6).map((event, index) => (
            <li key={index} className="flex flex-col gap-0.5">
              ...
              <div className="flex items-center gap-2">
                <span className="text-xs text-zinc-400">{event.categoryName}</span>
