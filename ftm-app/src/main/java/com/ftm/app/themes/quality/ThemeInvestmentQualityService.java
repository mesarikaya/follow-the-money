...
/**
 * Computes a single Investment Quality Score (IQS) — a 0-100 composite that combines signal quality
 * (confluence + persistence + streak) with portfolio attributes (value vs recent history,
...
public class ThemeInvestmentQualityService {
  ...
  private double computeSignalQuality(ThemeQualityContext ctx) {
    double streakContribution =
        Math.min(100, ctx.signalStreakDays() * (100.0 / MAX_STREAK_CONTRIBUTION));
