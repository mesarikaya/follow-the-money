...
/**
 * Stateless classifier implementing D-008 macro regime rules.
 * Evaluated in priority order; first matching condition wins.
 * <p>
 * Priority 1: T10Y2Y > 0.3 AND breakeven_inflation > 2.5 → STAGFLATION
 * Priority 2: VIX > 25 OR T10Y2Y < -0.2              → RISK_OFF_FLIGHT
 * Priority 3: T10Y2Y < 0.3 AND VIX < 22              → RISK_ON_DEFENSIVE
 * Priority 4: T10Y2Y > 0.3 AND VIX < 22              → RISK_ON_GROWTH
 * Fallback:   conflicting / missing                   → RISK_ON_GROWTH
 */
@Component
public class MacroRegimeClassifier {
    ...
    public MacroRegime classify(BigDecimal t10y2y,
                                BigDecimal vix,
                                BigDecimal breakEvenInflation) {
