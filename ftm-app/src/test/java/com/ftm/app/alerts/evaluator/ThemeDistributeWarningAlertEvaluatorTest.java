...
class ThemeDistributeWarningAlertEvaluatorTest {

  private static final LocalDate DATE = LocalDate.of(2024, 6, 1);
  private static final String RULE = "theme_distribute_warning";
  ...
  private ThemeDistributeWarningAlertEvaluator evaluator() {
    return new ThemeDistributeWarningAlertEvaluator(
  ...
  @Test
  void firesWhenStrongScoreButNegativeFlow() {
    ...
    when(signalRepository.findByTypeAndDate(SignalType.FLOW_20D, DATE))
        .thenReturn(Map.of("A", new BigDecimal("-1.0")));
  ...
  @Test
  void doesNotFireWhenFlowIsPositive() {
    when(alertRulesRepository.findById(RULE)).thenReturn(Optional.of(rule(true)));
    when(themeRepository.findAllConstituentsByTheme()).thenReturn(Map.of("AI", List.of("A")));
    when(signalRepository.findByTypeAndDate(SignalType.COMPOSITE, DATE))
        .thenReturn(Map.of("A", new BigDecimal("0.72")));
    when(signalRepository.findByTypeAndDate(SignalType.FLOW_20D, DATE))
        .thenReturn(Map.of("A", new BigDecimal("0.5")));
    lenient().when(alertRepository.existsActiveAlertForTheme(RULE, "AI")).thenReturn(false);
