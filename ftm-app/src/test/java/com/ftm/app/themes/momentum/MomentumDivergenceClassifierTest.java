...
class MomentumDivergenceClassifierTest {
  ...
  class AlignedBullish {
    ...
    void bothPositive() {
      assertThat(classifier.classify(0.008, 0.005)).hasValue(MomentumAlignment.ALIGNED_BULLISH);
    }

    @Test
    @DisplayName("both trends exactly at threshold → ALIGNED_BULLISH")
    void atThreshold() {
      assertThat(classifier.classify(0.003, 0.003)).hasValue(MomentumAlignment.ALIGNED_BULLISH);
    }
  }
  ...
  class Recovering {
    ...
    void shortTermDipInLongTermUptrend() {
      assertThat(classifier.classify(-0.005, 0.006)).hasValue(MomentumAlignment.RECOVERING);
    }
  }
  ...
  class Fading {
    ...
    void shortBounceinDowntrend() {
      assertThat(classifier.classify(0.006, -0.004)).hasValue(MomentumAlignment.FADING);
    }
  }
  ...
  class AlignedBearish {
    ...
    void bothNegative() {
      assertThat(classifier.classify(-0.007, -0.005)).hasValue(MomentumAlignment.ALIGNED_BEARISH);
    }
  }
  ...
  class Neutral {
    ...
    void shortTermFlat() {
      assertThat(classifier.classify(0.001, 0.006)).hasValue(MomentumAlignment.NEUTRAL);
    }

    @Test
    @DisplayName("5d positive, 20d below threshold → NEUTRAL")
    void longTermFlat() {
      assertThat(classifier.classify(0.006, 0.001)).hasValue(MomentumAlignment.NEUTRAL);
    }

    @Test
    @DisplayName("both near zero → NEUTRAL")
    void bothFlat() {
      assertThat(classifier.classify(0.001, -0.001)).hasValue(MomentumAlignment.NEUTRAL);
    }
  }
