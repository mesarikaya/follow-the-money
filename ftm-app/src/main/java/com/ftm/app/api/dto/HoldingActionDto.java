...
@Schema(description = "Recommended action for a single holding based on cross-referencing with signal engine")
public record HoldingActionDto(
    ...
    @Schema(description = "Short human-readable rationale explaining the recommendation")
        String rationale,
    @Schema(description = "Portfolio weight as a percentage of total EUR value; null if no price data")
        BigDecimal portfolioPct,
    @Schema(description = "Urgency order: lower = more urgent (EXIT=1, TRIM=2, WATCH=3, HOLD=4, UNCLASSIFIED=5)")
        int urgency) {}
