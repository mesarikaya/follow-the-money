...
class CacheEvictionListenerTest {
    ...
    void shouldEvictMacroCacheOnMacroPartial() {
        when(cacheManager.getCache("macro-latest")).thenReturn(macroCache);
        var event = new IngestionCompleteEvent(UUID.randomUUID(), IngestSource.MACRO, IngestStatus.PARTIAL);
    ...
    void shouldNotEvictCacheOnFailure() {
        var event = new IngestionCompleteEvent(UUID.randomUUID(), IngestSource.MACRO, IngestStatus.FAILED);
