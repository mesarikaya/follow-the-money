...
public class IngestionService {
  ...
  public IngestionService(
      PricesIngestionHandler pricesHandler,
  ...
  @TransactionalEventListener(
      ...
      condition = "#event.source().name() == 'PRICES'")
  @Async("asyncExecutor")
  ...
  @TransactionalEventListener(
      phase = TransactionPhase.AFTER_COMMIT,
      condition = "#event.source().name() == 'MACRO'")
  @Async("asyncExecutor")
  ...
  private void runIngestion(IngestSource source, Function<LocalDate, IngestionResult> handler) {
    ...
    try {
      ...
      log.info(
          "{} ingestion complete: {} rows, {} errors",
          source,
          result.rowsInserted(),
          result.errors().size());
    } catch (Exception ex) {
  ...
  private void finishAndPublish(
      IngestLog ingestLog, IngestSource source, IngestStatus status, int rows, String errorsJson) {
