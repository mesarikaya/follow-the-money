...
const REGIME_COLORS: Record<string, string> = {
  RISK_ON_GROWTH: "bg-green-900/60 text-green-300 border-green-700",
  RISK_ON_DEFENSIVE: "bg-yellow-900/60 text-yellow-300 border-yellow-700",
  RISK_OFF_GROWTH: "bg-orange-900/60 text-orange-300 border-orange-700",
  RISK_OFF_DEFENSIVE: "bg-red-900/60 text-red-300 border-red-700",
};
  ...
  return (
    <section className="space-y-4">
      <div className="flex items-center gap-3">
        <h2 className="text-base font-semibold text-zinc-200">Macro Environment</h2>
        <span className={`inline-block px-2.5 py-0.5 rounded border text-xs font-medium ${regimeClass}`}>
          {regime.replace(/_/g, " ")}
        </span>
        {asOfDate && (
          <span className="text-xs text-zinc-500">as of {asOfDate}</span>
        )}
      </div>
