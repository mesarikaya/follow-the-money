...
class MacroIndicatorRepositoryIT {
  ...
  @Autowired MacroIndicatorRepository repository;
  ...
  void shouldAllowNullValuePerSchema() {
    var row =
        Instancio.of(MacroIndicatorRepository.Row.class)
  ...
  void shouldReturnLatestObservationDateForSeries() {
    repository.batchInsert(
        List.of(row(DATE_1, SERIES_ID), row(DATE_3, SERIES_ID), row(DATE_2, SERIES_ID)));
  ...
  void shouldIsolateObservationDateLookupPerSeries() {
    repository.batchInsert(List.of(row(DATE_1, "DGS10"), row(DATE_3, "UNRATE")));
