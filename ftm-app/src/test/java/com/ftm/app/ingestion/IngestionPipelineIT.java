...
class IngestionPipelineIT {
  ...
  void setUp() {
    ...
    jdbcTemplate.execute(
        "TRUNCATE raw_prices, benchmark_prices, macro_indicators, ingest_log CASCADE");
  }
  ...
  void shouldPersistPricesAndCompleteSuccessfully() {
    ...
    var pricesLog =
        ingestLogRepository.findTopBySourceOrderByStartedAtDesc(IngestSource.PRICES).orElseThrow();
  ...
  void shouldPersistMacroDataAndCompleteSuccessfully() {
    ...
    await()
        ...
        .until(
            () ->
                ingestLogRepository
                    .findTopBySourceOrderByStartedAtDesc(IngestSource.MACRO)
                    .map(l -> l.status() != IngestStatus.RUNNING)
                    .orElse(false));

    var macroLog =
        ingestLogRepository.findTopBySourceOrderByStartedAtDesc(IngestSource.MACRO).orElseThrow();
  ...
  static class ExternalApiDispatcher extends Dispatcher {
    ...
    private static String fixture(String path) {
      try (var stream = ExternalApiDispatcher.class.getClassLoader().getResourceAsStream(path)) {
        return new String(
            Objects.requireNonNull(stream, "Fixture not found: " + path).readAllBytes(),
