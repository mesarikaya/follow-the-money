...
class ThemeInvestmentQualityServiceTest {
  ...
  void highQualityThemeGradesA() {
    ThemeQuality result = service.computeQuality(context(90, 85, 20, 0.01, 0.20, 0.10));
  ...
  void lowQualityThemeGradesFOrD() {
    ThemeQuality result = service.computeQuality(context(15, 10, 0, 0.09, 0.90, 0.95));
  ...
  void nullInputsUseNeutralFallback() {
    ThemeQuality result = service.computeQuality(context(50, 50, 0, null, null, null));
  ...
  void cheapThemeScoresHigherThanExpensive() {
    ThemeQuality cheapResult = service.computeQuality(context(60, 60, 10, 0.02, 0.40, 0.05));
    ThemeQuality expensiveResult = service.computeQuality(context(60, 60, 10, 0.02, 0.40, 0.95));
  ...
  void concentratedThemeScoresLowerThanDiversified() {
    ThemeQuality concentrated = service.computeQuality(context(60, 60, 10, 0.02, 0.95, 0.50));
    ThemeQuality diversified = service.computeQuality(context(60, 60, 10, 0.02, 0.20, 0.50));
  ...
  void gradeBoundaries() {
    // score = signalQuality(92)*0.50 + valueScore(95)*0.20 + diversification(90)*0.15 +
    // volatility(90)*0.15 = 92
    assertThat(
            service.computeQuality(context(90, 90, 30, 0.01, 0.10, 0.05)).investmentQualityGrade())
        .isEqualTo("A");
    assertThat(
            service.computeQuality(context(40, 40, 5, 0.05, 0.50, 0.50)).investmentQualityGrade())
        .isIn("B", "C");
  }
}
