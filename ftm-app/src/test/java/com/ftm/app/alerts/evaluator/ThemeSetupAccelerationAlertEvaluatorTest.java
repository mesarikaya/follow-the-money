...
class ThemeSetupAccelerationAlertEvaluatorTest {

  private static final LocalDate DATE = LocalDate.of(2024, 6, 1);
  private static final String RULE = "theme_setup_acceleration";
  ...
  private ThemeSetupAccelerationAlertEvaluator evaluator() {
    return new ThemeSetupAccelerationAlertEvaluator(
  ...
  @Test
  void firesWhenInSetupBandAndAccelerating() {
    ...
    when(signalRepository.findByTypeAndDate(SignalType.COMPOSITE, DATE))
        .thenReturn(Map.of("A", new BigDecimal("0.58")));
  ...
  @Test
  void doesNotFireWhenAlreadyInBuyZone() {
    when(alertRulesRepository.findById(RULE)).thenReturn(Optional.of(rule(true)));
    when(themeRepository.findAllConstituentsByTheme()).thenReturn(Map.of("AI", List.of("A")));
    when(signalRepository.findByTypeAndDate(SignalType.COMPOSITE, DATE))
