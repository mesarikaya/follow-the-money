...
@Component
public class SteadyUptrendRule implements EntryTimingRule {
  ...
  public Optional<EntryRecommendation> evaluate(EntryTimingContext context) {
    ...
    boolean calmPositiveShortTerm = trend5d != null && trend5d >= 0;
    boolean calmPositiveLongTerm = trend20d != null && trend20d >= 0;
    boolean safeRisk = SAFE_RISK_LEVELS.contains(context.riskLevel());
    if (inBuyZone && calmPositiveShortTerm && calmPositiveLongTerm && safeRisk) {
      return Optional.of(
          new EntryRecommendation(
              EntryAction.ENTER, "Steady BUY zone — positive momentum across both timeframes"));
    }
