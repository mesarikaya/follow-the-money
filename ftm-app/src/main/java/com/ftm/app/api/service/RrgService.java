...
public class RrgService {
    ...
    private static final List<String> PALETTE = List.of(
            ...
            "#14B8A6", "#F43F5E", "#22D3EE", "#A855F7", "#FCD34D",
            "#34D399", "#FB7185", "#60A5FA", "#A3E635", "#FBBF24"
    );
    ...
    public RrgResponse getLatest() {
        ...
        Map<String, Category> categoryById = categoryRepository.findAllByActiveTrueOrderByDisplayOrderAsc()
                .stream()
                .collect(Collectors.toMap(c -> c.id().name(), c -> c));

        LocalDate latestDate = rows.stream()
                .map(SignalRepository.RrgRow::signalDate)
                .max(LocalDate::compareTo)
                .orElse(LocalDate.now());

        List<RrgCategoryEntry> entries = rows.stream()
    ...
    private RrgCategoryEntry buildEntry(String catId, List<SignalRepository.RrgRow> rows, Category cat) {
        Map<LocalDate, BigDecimal> ratioByDate = rows.stream()
                .filter(r -> r.signalType() == SignalType.RRG_RATIO)
                .collect(Collectors.toMap(SignalRepository.RrgRow::signalDate, SignalRepository.RrgRow::value));

        Map<LocalDate, BigDecimal> momByDate = rows.stream()
                .filter(r -> r.signalType() == SignalType.RRG_MOM)
                .collect(Collectors.toMap(SignalRepository.RrgRow::signalDate, SignalRepository.RrgRow::value));

        int latestQuadrant = rows.stream()
        ...
        List<RrgTrailPoint> trail = ratioByDate.keySet().stream()
