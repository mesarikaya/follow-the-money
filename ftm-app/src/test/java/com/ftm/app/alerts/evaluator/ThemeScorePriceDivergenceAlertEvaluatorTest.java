...
class ThemeScorePriceDivergenceAlertEvaluatorTest {

  private static final LocalDate DATE = LocalDate.of(2024, 6, 1);
  private static final String RULE = "theme_score_price_divergence";
  ...
  private ThemeScorePriceDivergenceAlertEvaluator evaluator() {
    return new ThemeScorePriceDivergenceAlertEvaluator(
  ...
  @Test
  void firesWhenBullishScoreButNegativeRs() {
  ...
  @Test
  void doesNotFireWhenRsIsPositive() {
    when(alertRulesRepository.findById(RULE)).thenReturn(Optional.of(rule(true)));
    when(themeRepository.findAllConstituentsByTheme()).thenReturn(Map.of("AI", List.of("A", "B")));
    when(signalRepository.findByTypeAndDate(SignalType.COMPOSITE, DATE))
        .thenReturn(Map.of("A", new BigDecimal("0.70"), "B", new BigDecimal("0.70")));
