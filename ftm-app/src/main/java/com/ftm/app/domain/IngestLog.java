...
public record IngestLog(
    ...
    IngestSource source) {
  public IngestLog(
      OffsetDateTime startedAt, IngestStatus status, Integer rowsInserted, IngestSource source) {
    this(UUID.randomUUID(), startedAt, null, status, rowsInserted, null, source);
  }

  public IngestLog finish(
      OffsetDateTime finishedAt, IngestStatus status, Integer rowsInserted, String errors) {
