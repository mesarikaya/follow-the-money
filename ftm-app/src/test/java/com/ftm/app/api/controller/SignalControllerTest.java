...
import com.ftm.app.domain.Signal;
...
class SignalControllerTest {
  ...
  void setUp() {
    mockMvc =
        MockMvcBuilders.standaloneSetup(
                new SignalController(signalRepository, signalHistoryMapper, signalComputationService))
  ...
  void shouldReturnSignalHistory() throws Exception {
    Signal signal =
        Instancio.of(Signal.class)
            .set(field(Signal::categoryId), CategoryId.TECH)
            .set(field(Signal::signalType), SignalType.COMPOSITE)
            .set(field(Signal::signalDate), DATE)
            .set(field(Signal::value), new BigDecimal("0.750"))
            .create();
    SignalHistoryDto dto =
        new SignalHistoryDto(DATE, SignalType.COMPOSITE, new BigDecimal("0.750"), signal.computedAt());

    when(signalRepository.findByCategoryId("TECH")).thenReturn(List.of(signal));
    when(signalHistoryMapper.toDto(signal)).thenReturn(dto);
