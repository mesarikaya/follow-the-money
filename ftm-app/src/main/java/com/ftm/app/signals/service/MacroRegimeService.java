...
/**
 * Applies the D-008 macro regime classification rules to live FRED data. Also computes the
 * MACRO_FIT win-rate per category: the fraction of historical days under the current regime where
 * RS_60 was positive.
 */
@Service
public class MacroRegimeService {
  ...
  private static final Set<String> REQUIRED_SERIES =
      Set.of(
  ...
  /**
   * Computes the MACRO_FIT win-rate per category for the given regime. Win-rate = fraction of
   * historical days under that regime where RS_60 was positive (> 0). Returns a value in [0.0, 1.0]
   * per category, or null if insufficient data.
   */
  public Map<String, BigDecimal> computeMacroFitByCategory(MacroRegime currentRegime) {
    ...
    if (datesMatchingCurrentRegime.isEmpty()) {
      log.warn(
          "No historical dates found matching regime {}; MACRO_FIT will be empty", currentRegime);
      return Map.of();
    }

    log.debug(
        "Found {} historical dates matching regime {}",
        datesMatchingCurrentRegime.size(),
        currentRegime);
  ...
  private Map<String, BigDecimal> computeWinRates(
      List<LocalDate> regimeDates,
      Map<LocalDate, Map<String, BigDecimal>> relativeStrength60DayByDate) {
    Map<String, Integer> positiveCount = new HashMap<>();
    Map<String, Integer> totalCount = new HashMap<>();

    for (LocalDate date : regimeDates) {
      Map<String, BigDecimal> relativeStrength60DayByCategory =
          relativeStrength60DayByDate.get(date);
    ...
    for (String categoryId : totalCount.keySet()) {
      ...
      winRates.put(
          categoryId,
          BigDecimal.valueOf(positive).divide(BigDecimal.valueOf(total), 6, RoundingMode.HALF_UP));
    }
