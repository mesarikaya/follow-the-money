...
class IngestControllerTest {
  ...
  void setUp() {
    mockMvc =
        MockMvcBuilders.standaloneSetup(new IngestController(ingestTriggerService))
  ...
  void shouldReturn202WithRunIds() throws Exception {
    ...
    when(ingestTriggerService.trigger())
        .thenReturn(
            new IngestTriggerResponse(
                List.of(runId, UUID.randomUUID()), "queued", "Ingestion started"));

    mockMvc
        .perform(post("/ingest/trigger"))
  ...
  void shouldReturn200ForExistingRun() throws Exception {
    UUID runId = UUID.randomUUID();
    when(ingestTriggerService.getStatus(runId))
        .thenReturn(statusResponse(runId, "prices", "success"));

    mockMvc
        .perform(get("/ingest/status/{runId}", runId))
  ...
  void shouldReturn404WhenRunNotFound() throws Exception {
    ...
    mockMvc.perform(get("/ingest/status/{runId}", runId)).andExpect(status().isNotFound());
  }
  ...
  void shouldReturn200WithLatestLogList() throws Exception {
    when(ingestTriggerService.getLatestPerSource())
        .thenReturn(
            List.of(
                statusResponse(UUID.randomUUID(), "prices", "success"),
                statusResponse(UUID.randomUUID(), "macro", "running")));

    mockMvc
        .perform(get("/ingest/status/latest"))
        .andExpect(status().isOk())
