...
public class ThemeService {
  ...
  public List<ThemeSummaryDto> getThemes() {
    ...
    return themes.stream()
        .map(theme -> {
          ...
          List<ThemeConstituentDto> allConstituents = buildConstituents(ids, categoriesById, signals);
          List<ThemeConstituentDto> top3 = allConstituents.stream()
  ...
  public ThemeDetailDto getTheme(String themeId) {
    ...
    List<ThemeConstituentDto> sorted = constituents.stream()
  ...
  private Map<SignalType, Map<String, BigDecimal>> fetchSignals() {
    return signalRepository.findLatestByTypes(List.of(
  ...
  private List<ThemeConstituentDto> buildConstituents(
    ...
    Map<String, BigDecimal> compositeMap = signals.getOrDefault(SignalType.COMPOSITE, Collections.emptyMap());
    Map<String, BigDecimal> rs60Map = signals.getOrDefault(SignalType.RS_60, Collections.emptyMap());
    Map<String, BigDecimal> flow20dMap = signals.getOrDefault(SignalType.FLOW_20D, Collections.emptyMap());
    Map<String, BigDecimal> trend20dMap = signals.getOrDefault(SignalType.COMPOSITE_TREND_20D, Collections.emptyMap());
    Map<String, BigDecimal> rrgMap = signals.getOrDefault(SignalType.RRG_QUADRANT, Collections.emptyMap());
    Map<String, BigDecimal> macroFitMap = signals.getOrDefault(SignalType.MACRO_FIT, Collections.emptyMap());
    Map<String, BigDecimal> rs120Map = signals.getOrDefault(SignalType.RS_120, Collections.emptyMap());
    Map<String, BigDecimal> trend5dMap = signals.getOrDefault(SignalType.COMPOSITE_TREND_5D, Collections.emptyMap());
    ...
    return categoryIds.stream().map(id -> {
      ...
      int conviction = TradeSignalDeriver.convictionScore(
          composite, rrg, trend20d,
          macroFitMap.get(id), null,
          trend5dMap.get(id), rs60, rs120Map.get(id), flow20d, null);
      return new ThemeConstituentDto(
          id, name, ticker,
          composite, rs60, flow20d, trend20d,
  ...
  private ThemeSummaryDto toSummary(
    ...
    OptionalDouble avgComposite = allConstituents.stream()
        ...
        .average();
    OptionalDouble avgRs60 = allConstituents.stream()
        .filter(c -> c.rs60() != null)
        .mapToDouble(c -> c.rs60().doubleValue())
        .average();
    OptionalDouble avgFlow = allConstituents.stream()
        .filter(c -> c.flow20d() != null)
        .mapToDouble(c -> c.flow20d().doubleValue())
        .average();
    OptionalDouble avgTrend = allConstituents.stream()
    ...
    long reduceCount = allConstituents.stream().filter(c -> "REDUCE".equals(c.tradeSignal())).count();
