 ...
 *           + 0.20 × norm(MOM)
 *           + 0.10 × norm(MACRO_FIT)
 *           + 0.10 × relativeRotationGraphScore
 *
 * norm() = min-max normalization across all categories on a given date.
 * relativeRotationGraphScore: Leading=1.0, Improving=0.7, Weakening=0.3, Lagging=0.0.
 * Null components are excluded and remaining weights are redistributed proportionally.
 */
@Service
public class CompositeScoreService {
    ...
    public Map<String, BigDecimal> computeCompositeScores(
        ...
        Set<String> allCategoryIds = collectAllCategoryIds(
                rs60ByCategoryId, flow20DayByCategoryId, momentumByCategoryId,
                macroFitByCategoryId, rrgQuadrantByCategoryId);
        ...
        Map<String, BigDecimal> relativeRotationGraphScores = deriveRrgScores(rrgQuadrantByCategoryId, allCategoryIds);

        Map<String, BigDecimal> compositeScores = new HashMap<>();
        for (String categoryId : allCategoryIds) {
            BigDecimal score = computeWeightedScore(
    ...
    private BigDecimal computeWeightedScore(BigDecimal normalizedRs60,
    ...
    private Map<String, BigDecimal> normalize(Map<String, BigDecimal> rawValues,
        ...
        java.util.List<BigDecimal> nonNulls = filled.values().stream()
        ...
        for (Map.Entry<String, BigDecimal> entry : filled.entrySet()) {
            ...
            if (value == null) {
                ...
                normalized.put(entry.getKey(),
                        value.subtract(min).divide(range, 6, RoundingMode.HALF_UP));
            }
        }
        return normalized;
    }

    private Map<String, BigDecimal> deriveRrgScores(Map<String, BigDecimal> quadrantByCategoryId,
                                                    Set<String> allCategoryIds) {
