...
public class RotationService {
    ...
    public RotationService(CategoryRepository categoryRepository,
    ...
    public RotationResponse getLatest() {
        ...
        Map<String, BigDecimal> compositeScores        = signalRepository.findLatestByType(SignalType.COMPOSITE);
        Map<String, BigDecimal> relativeStrength60Days = signalRepository.findLatestByType(SignalType.RS_60);
        Map<String, BigDecimal> rrgQuadrants           = signalRepository.findLatestByType(SignalType.RRG_QUADRANT);

        LocalDate asOfDate = compositeScores.isEmpty() ? LocalDate.now()
                : signalRepository.findLatestByType(SignalType.COMPOSITE).entrySet().stream()
                        .findFirst()
                        .map(e -> LocalDate.now())
                        .orElse(LocalDate.now());

        List<RotationLeaderEntry> allRanked = categoriesById.entrySet().stream()
                ...
                .sorted(Comparator.comparing(RotationLeaderEntry::compositeScore,
                        Comparator.nullsLast(Comparator.reverseOrder())))
                .toList();

        List<RotationLeaderEntry> topLeaders = allRanked.stream()
                .limit(ROTATION_LEADER_COUNT)
                .toList();

        List<RotationLeaderEntry> bottomLaggards = allRanked.reversed().stream()
                .limit(ROTATION_LEADER_COUNT)
                .toList();

        List<RotationEvent> recentEvents = rotationEventRepository.findRecentEvents(
                LocalDate.now().minusDays(RECENT_EVENTS_LOOKBACK_DAYS));

        List<RotationEventEntry> eventEntries = recentEvents.stream()
                .map(event -> {
                    Category category = categoriesById.get(event.categoryId().name());
                    String categoryName = category != null ? category.name() : event.categoryId().name();
